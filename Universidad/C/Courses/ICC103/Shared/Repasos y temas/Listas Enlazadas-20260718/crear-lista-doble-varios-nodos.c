...
void GuardarPersonaArchivo(NODOPERSONA *, char *);
...
int main()
{
    ...
    for(i=0; i<n; i++)
    {
        ...
        if(i==0)
        {
         ...
         GuardarPersonaArchivo(cabeza,"personas.dat");
    }
...
NODOPERSONA* CrearNodoCabezaPersona(INFOPERSONA persona, NODOPERSONA *cabeza)
{
    ...
    if(pnuevo == NULL)
    {
        printf("\nError en la asignaci%cn de memoria\n",162);
        return NULL;
    }
...
void GuardarPersonaArchivo(NODOPERSONA *cabeza, char *file_name)
{
    FILE *fpersona;
    ...
    NODOPERSONA *indice;
    for(indice=cabeza; indice; indice=indice->proximo)
        fwrite(&indice->persona,sizeof(INFOPERSONA),1,fpersona);
    }
